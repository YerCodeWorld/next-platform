...
interface MultipleChoiceCardsDisplayProps {
  exercise: Exercise;
...
export function MultipleChoiceCardsDisplay({
  exercise,
  package: pkg,
  locale,
  userData,
  onComplete
}: MultipleChoiceCardsDisplayProps) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [questionStates, currentQuestionIndex, isMultipleChoice, showResults, currentState]);
  ...
  // Check if option contains an image URL
  const isImageOption = (option: string): boolean => {
    return option.includes('http') || option.includes('data:image') || 
           option.includes('.jpg') || option.includes('.png') || 
           option.includes('.gif') || option.includes('.webp');
  };
  ...
  return (
    <div className={`multiple-choice-display multiple-choice-cards-display question-${currentQuestionIndex % 10}`}>
      ...
      <div className="mc-container">
        ...
        >
          ...
          >
            {currentQuestion.question || exercise.instructions || 'Select the correct answer'}
          </motion.div>
        ...
        {/* Cards Options Grid */}
        <motion.div 
          className={`mc-cards-grid ${currentQuestion.options.some(isImageOption) ? 'has-images' : ''}`}
          variants={staggerContainer}
          initial="hidden"
          animate="visible"
          key={`cards-${currentQuestionIndex}`}
        >
            ...
            {currentQuestion.options.map((option, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`mc-card-option ${optionClass} ${isImage ? 'image-card' : 'text-card'}`}
                ...
                >
                  {isImage ? (
                    <div className="mc-card-image-container">
                      <img 
                        src={option} 
                        alt={`Option ${index + 1}`}
                      ...
                      />
                      {showResults && (
                        <div className="mc-card-result-icon">
                          {currentQuestion.correctIndices.includes(index) ? (
                            <CheckCircle className="text-green-600" />
                          ) : currentState.selectedOptions.includes(index) ? (
                    ...
                    <div className="mc-card-text-container">
                      <div className="mc-card-text">{option}</div>
                      {showResults && (
                        <div className="mc-card-result-icon">
                          {currentQuestion.correctIndices.includes(index) ? (
                            <CheckCircle className="text-green-600" />
